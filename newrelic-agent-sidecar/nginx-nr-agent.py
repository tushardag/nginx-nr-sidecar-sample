...
# 
# Updated by Tushar Dagade to work in 
# PCF environment as Sidecar with environment variables
...
LOG.setLevel(os.getenv('NEWRELIC_PLUGIN_LOG_LEVEL', 'ERROR'))
...
class NginxNewRelicAgent():
    ...
    def newrelic_push(self):
        ...
        payload = dict()
        payload['agent'] = { 'version': AGENT_VERSION, 'host': AGENT_HOSTNAME }
    ...
    def create_config(self):
        if self.license_key:
            return
        self.license_key = os.getenv('NEWRELIC_LICENSE_KEY', "NONE")
        ns = NginxApiCollector(self, os.getenv('NEWRELIC_APP_NAME','NGINX_App'), "http://localhost:8080/nginx_stub_status", self.poll_interval)
        if os.getenv('NGINX_HTTP_USER') and os.getenv('NGINX_HTTP_PWD'):
            ns.basic_auth = base64.b64encode(os.getenv('NGINX_HTTP_USER') + b':' + os.getenv('NGINX_HTTP_PWD'))
